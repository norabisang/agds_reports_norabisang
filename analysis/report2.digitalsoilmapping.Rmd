...
No missing data in the covariate data. (Mostly sampled data, pH and timeset data is missing.) 
Missing data is mostly from the same entries, by keeping only entries where there is pH data, this will give a dataset with pracitally no missing data.
...
Now we only stack the selected covariates
```{r}
...
The OOB values
```{r}
rf_basic$prediction.error   # OOB error for full model
...
The fitted and tested model will now be used for spatial upscaling - creation of a map of waterlogging at 100cm soil depth across our study area.
```{r}
