---
title: "AGDS Report 2 - digital soil mapping"
author: "Nora Bisang"
...
### 5.3 Retiring Confirmed variable
...
df_test$pred_boruta_class <- factor(df_test$pred_boruta_class, levels = c(0, 1))
df_test$pred_basic_class <- factor(df_test$pred_basic_class, levels = c(0, 1))
...
### 9.1 Cross-validation preparation
```{r}
# Select predictors
...
### 9.3 Model tuning and tuning results
...
# Convert probabilities to classes using optimal threshold for avoiding arbitrary 0.5 cutoff. 
#If the predicted probability higher this threshold, classified waterlogged, else not waterlogged
...
Comparing the two models, the Untuned Model performs better across all RF-default metrics. It is ~ 1.5% more accurate. That the 95% CIs overlap (72.7%–84.4% for the untuned model and 71.1%-83.1% for the tuned model) tells us that though the untuned model is better, both models perform similarly well and are stable and reliable. 
For the final mapping of waterlogging at 100 cm depth, the default settings provided the most reliable results and though this model should be used for spatial upscaling.
...
Brier Score
```{r}
rf_prob_adj$prediction.error 
```



Interpretation Roc/ AUC
The hyperparameter tuned probability model performs well above random and good with an AUC of  0.868. In soil modeling, an AUC above 0.80 is generally considered a high-quality result.
The models Brier Score of 0.143 is well below the 0.25 (model with no predictive power), which confirms its reliability. 
These values indicate that the model is not simply guessing categories, but rather provides informative probability estimates for waterlogging at 100 cm depth. 
...
How would you chose the threshold in each case? Would you chose the same threshold in both cases? If not, explain why. 

I would choose the threshold for the cases based on the relative costs/risks of the error types false positive / false negative. For the "safety critical building site" (case 1) I would choose a lower threshold to avoid false negatives more strictly. For non-critical cases I would choose a higher threshold (to avoid unneccessary and costly intervention). There are algorithms that one can use to estimate appropriate thresholds like the Bayes Decision Theory (BDT) on which I would base this decision. 
...
A similarly natured problem could be the screening for a severe or contageant disease, that needs immediate treatment, where the cost of a false negative (missing a diseased person) would be very big. Whereas for the screening for a non severe and less contageant elective issue, I would set  the  threshold higher to avoid false positives and unnecessary invasive follow-up.
