...
readr::write_csv(df_full, "data-raw/df_full.csv")
...
# ensure aimed file exists
if (!dir.exists(dst)) {
  dir.create(dst, recursive = TRUE)
  message("target file 'data-raw' was created.")
}
...
# Load all files as one batch
all_rasters <- rast(list_raster) #here I have very different max and min than in tutorial book this probably as a random chosen subset
all_rasters
...
NOW ONTO MODEL TESTING: MAKE PREDICTIONS
Exercize 5.2
...
Now Metrics for comparison of the 2 models: 
Here confusion matrix for boruta
...
It improves the accuracy, kappa, sensitivity, specificity, and balanced accuracy. -> strong indication that predictor selection by Boruta was useful to remove noisy / irrelevant predictors, improving the performance on unseen data.
...
```
After seeing the OOB I would still choose the Boruta model, which is slightly better also in OOB. 
...
We want to define cross-validation and hyperparameter grid
mtry: number of variables randomly sampled at each split.
min.node.size: minimum number of samples in a terminal node.
We dont use MAE as this is for regressions i use accuracy
...
#we define train and test using the bor 
...
#to ensure that 1 is considered positive
y_train <- factor(y_train, levels = c("1", "0"), labels = c("Yes", "No"))
y_test  <- factor(y_test,  levels = c("1", "0"), labels = c("Yes", "No")) #for caret



#next we could standardize numeric variables (mean = 0, sd = 1) Important for some models like KNN, not necessary for Random Forest so I don do it. 
...
Bestes-Tune-results: 
mtry = 6: At each split algorithm considered 6 random predictors.

splitrule = "gini": Standard Gini impurity used to choose splits.

min.node.size = 3: Terminal nodes allowed to contain as few as 5 observations
...
roc_test <- roc(y_test, prob.test[, "Yes"])
...
```
Answer 5.3 Does the model with adjusted hyperparameters perform better? 
I here compare the models with the boruta selected parameters. 
The model without cross-validation and mtry and min.node.size tuning. 
...
Probability= T
Train new Random Forest with probability = True to get a probability map from 0-100% 

```{r}
library(ranger)
rf_basic.prob <- ranger(
  y = df_train[, target],     # target variable
  x = df_train[, predictors_all], 
  seed = 42,# Specify the seed for randomization to reproduce the same model again
  num.threads = parallel::detectCores() - 1, 
...
# Print a summary of fitted model
print(rf_basic.prob) # we have a classification as factors w. 2 levels #target Node size 1
...
colnames(rf_bor.prob$predictions)
#extraction of the probability of 1
probs.1 <- rf_bor.prob$predictions[, "1"]
```
...
# quick report and performance of trained model object
rf_bor.prob.adj #OOB prediction error (Brier s.):  0.1434678

probs.adj <- rf_bor.prob.adj$predictions[, "1"]
...
# Plot ROC curve
plot(roc_obj.bor.adj, col = "blue", lwd = 2, main = "ROC Curve - Boruta RF Adjusted (OOB)")

# AUC
auc(roc_obj.bor.adj) #Area under the curve: 0.8648 this is good. 
...
# Plot ROC curve
plot(roc_obj.bor, col = "blue", lwd = 2, main = "ROC Curve - Boruta RF (OOB)")

# AUC
auc(roc_obj.bor) #Area under the curve: 0.8632 this is good. 
...
I will look at the ROC area under curve for both for comparison of the model performance

```{r}
{plot(roc_obj.bor, col="blue", main="ROC: Hyperparameters Non-adjusted vs adjusted")
lines(roc_obj.bor.adj, col="red") 
legend("bottomright", legend=c("RF Prob. NA","RF Prob. A"), col=c("blue","red"), lwd=2)}
...
```{r}
rf_bor.prob$prediction.error # OOB error for bor: undadjusted hyperparameters = 0.1442626
rf_basic.prob$prediction.error # OOB error for basic unadjusted hyper = 0.1508364
rf_bor.prob.adj$prediction.error # OOB error for bor: undadjusted hyperparameters = 0.1434678
```
